...
const AVATAR_COLORS = [
  '#7C5CFC', '#E01E5A', '#2BAC76', '#E8912D',
  '#1264A3', '#9C27B0', '#00B6C6', '#D1707B',
]
...
export function CallPanel() {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex flex-col" style={{ background: '#16181D' }}>
      <LiveKitRoom
        serverUrl={callUrl}
        token={callToken}
        connect
  ...
  return (
    ...
    >
      {hasCamera ? (
        ...
        <div className="h-full w-full flex items-center justify-center" style={{ minHeight: 180 }}>
          <div
      ...
      >
        {!participant.isMicrophoneEnabled && <MicOff className="h-3 w-3 shrink-0" style={{ color: '#E01E5A' }} />}
        <span className="text-[12px] font-medium truncate" style={{ color: '#ffffff' }}>
          {name}
...
function CallContent({
  ...
  const cameraTracks = useMemo(
    () => tracks.filter((t) => t.source === Track.Source.Camera && isTrackReference(t)),
    [tracks]
  )
  const screenTracks = useMemo(
    () => tracks.filter((t) => t.source === Track.Source.ScreenShare && isTrackReference(t)),
    [tracks]
  )
  const audioTracks = useMemo(
    () => tracks.filter((t) => t.source === Track.Source.Microphone && isTrackReference(t)),
    [tracks]
  )
  ...
  return (
    <div className="h-full flex flex-col min-h-0">
      ...
      <div
        ...
        style={{ background: '#0F1115', borderBottom: '1px solid rgba(255,255,255,0.07)' }}
      >
        <div
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              style={{
                background: isVideoCall ? 'rgba(124,92,252,0.18)' : 'rgba(43,172,118,0.15)',
          ...
          <div className="flex items-center gap-1.5">
            <span className="h-1.5 w-1.5 rounded-full animate-pulse" style={{ background: '#E01E5A' }} />
      ...
      <div className="flex-1 flex min-h-0">
        <div className="flex-1 flex flex-col min-w-0 p-4 gap-3">
          {spotlight && (
            <div className="h-[45%] shrink-0 rounded-xl overflow-hidden" style={{ background: '#15181E' }}>
              <VideoTrack trackRef={spotlight} className="h-full w-full object-contain" />
            </div>
          )}
      ...
      <div
        ...
        style={{ background: '#0F1115', borderTop: '1px solid rgba(255,255,255,0.07)' }}
      >
        <ControlButton
          onClick={toggleMute}
          active={!isMuted}
...
function PeopleDrawer({
  ...
  async function handleRing(profile: Profile) {
    ...
    setRingStatus((prev) => ({ ...prev, [profile.id]: ok ? 'sent' : 'failed' }))
  }
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        <div className="px-2 py-2">
          ...
          {participants.map((p) => (
            <div key={p.identity} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg">
              <div
              ...
              {p.muted ? (
                <MicOff className="h-3.5 w-3.5 shrink-0" style={{ color: '#E01E5A' }} />
              ) : (
                <Mic className="h-3.5 w-3.5 shrink-0" style={{ color: 'rgba(255,255,255,0.4)' }} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="px-2 py-2" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          {loadingMembers ? (
            ...
            <div className="px-3 py-3 text-[12px]" style={{ color: 'rgba(255,255,255,0.45)' }}>
              {members.length === 0
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {status === 'sent' ? 'Rung' : status === 'failed' ? 'Offline' : 'Ring'}
                    </button>
                  </div>
                )
              })}

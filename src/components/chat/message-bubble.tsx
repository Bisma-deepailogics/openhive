...
export function MessageBubble({
  ...
  useEffect(() => {
    ...
    const uncached = uuids.filter(
      (id) => !mentionNameCache[id]
    )
  ...
  async function handleDelete() {
    ...
    try {
      ...
      // Immediately notify ThreadPanel / channel UI
      ...
      >
        ...
        {showHeader ? (
          ...
          <div className="w-9 shrink-0 flex items-center justify-center">
            <span
              className={`text-[10px] text-[#8E8EA0] transition-opacity ${
                showToolbar
                  ? 'opacity-100'
                  : 'opacity-0'
              }`}
